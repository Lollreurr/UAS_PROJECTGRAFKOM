...
public class Cone extends OOval{

    float radiusZ;
    List<Integer> index;
    int ibo;

    public Cone(List<ShaderModuleData> shaderModuleDataList, List<Vector3f> vertices, Vector4f color, Vector3f centerPoint,
    ...
    public void createEllipticParaboloid(){
        ...
        for(double v = -Math.PI/2; v<= Math.PI/2; v+=Math.PI/60){
            for(double u = -Math.PI; u<= Math.PI; u+=Math.PI/60){
                ...
                float z = (float) Math.pow((float)(v),radiusZ);
